...
int ws_handshake(tls_conn *c, const char *host, const char *path) {
    ...
    if (random_bytes(key_raw, sizeof(key_raw)) != 0) return -1;
    ...
    if (tls_write(c, req, (size_t)n) != 0) return -1;
...
int ws_send_text(tls_conn *c, const char *text, size_t len) {
    ...
    if (random_bytes(mask, 4) != 0) return -1;
...
int ws_read_text(tls_conn *c, char *out, size_t out_cap, size_t *out_len) {
    unsigned char hdr[2];
    if (tls_read_exact(c, hdr, 2) != 0) return -1;
    ...
    if (plen == 126) {
        unsigned char ext[2];
        if (tls_read_exact(c, ext, 2) != 0) return -1;
        plen = ((uint64_t)ext[0] << 8) | ext[1];
    } else if (plen == 127) {
        unsigned char ext[8];
        if (tls_read_exact(c, ext, 8) != 0) return -1;
        plen = 0;
        for (int i = 0; i < 8; i++) plen = (plen << 8) | ext[i];
    }

    unsigned char mask[4] = {0};
    if (masked && tls_read_exact(c, mask, 4) != 0) return -1;
    ...
    if (plen + 1 > out_cap) {
        ...
        while (rem > 0) {
            ...
            if (tls_read_exact(c, tmp, chunk) != 0) return -1;
            rem -= chunk;
        }
        return -1;
    }

    if (tls_read_exact(c, (unsigned char *)out, (size_t)plen) != 0) return -1;
