...
int tls_init_ca(void) {
    ...
    int rc = mbedtls_x509_crt_parse(&g_ca,
...
int tls_connect(tls_conn *c, const char *host, const char *port) {
    ...
    if (mbedtls_ctr_drbg_seed(&c->drbg, entropy_func, &c->entropy,
                               (const unsigned char *)"king", 4) != 0) {
        goto fail;
    }
    ...
    if (mbedtls_ssl_set_hostname(&c->ssl, host) != 0) goto fail;
...
int https_request(const char *host, const char *port, const char *req,
    ...
    tls_conn c;
    if (tls_connect(&c, host, port) != 0) return -1;
